...
public class NoteController implements Initializable {
    ...
    @Override public void initialize(URL location,ResourceBundle resources){
        ...
        tableNotes.setItems(data);tableNotes.getSelectionModel().selectedItemProperty().addListener((obs,old,val)->{if(val!=null)fillForm(val);});refresh();
    }
    private void fillForm(Note n){txtAnnee.setText(n.getAnneeScolaire());txtNote.setText(String.valueOf(n.getNote()));cbEleve.getItems().stream().filter(e->e.getNumEleve().equals(n.getNumEleve())).findFirst().ifPresent(cbEleve::setValue);cbMatiere.getItems().stream().filter(m->m.getNumMat().equals(n.getNumMat())).findFirst().ifPresent(cbMatiere::setValue);}
    @FXML private void handleAjouter(){Note n=new Note(txtAnnee.getText(),cbEleve.getValue().getNumEleve(),cbMatiere.getValue().getNumMat(),Double.parseDouble(txtNote.getText()));noteService.ajouter(n);refresh();handleClear();}
    @FXML private void handleModifier(){Note n=new Note(txtAnnee.getText(),cbEleve.getValue().getNumEleve(),cbMatiere.getValue().getNumMat(),Double.parseDouble(txtNote.getText()));noteService.modifier(n);refresh();}
    @FXML private void handleSupprimer(){noteService.supprimer(txtAnnee.getText(),cbEleve.getValue().getNumEleve(),cbMatiere.getValue().getNumMat());refresh();handleClear();}
    @FXML private void handleClear(){txtAnnee.clear();txtNote.clear();cbEleve.setValue(null);cbMatiere.setValue(null);}
