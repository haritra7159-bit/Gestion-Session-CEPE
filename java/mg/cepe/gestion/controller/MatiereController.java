...
public class MatiereController implements Initializable {
    @FXML
    private ComboBox<String> cbNumMat;
    @FXML
    private TextField txtDesignMat;
    @FXML
    private Spinner<Double> spCoef;
    @FXML
    ...
    public void initialize(URL u, ResourceBundle r) {
        ...
        if (spCoef.getValueFactory() == null) {
            spCoef.setValueFactory(
                    new SpinnerValueFactory.DoubleSpinnerValueFactory(0.25, 10.0, 0.5, 0.25));
        }

        colNum.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getNumMat()));
        colDesign.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getDesignMat()));
        colCoef.setCellValueFactory(c -> new SimpleDoubleProperty(c.getValue().getCoef()));
    ...
    private void handleAjouter() {
        ...
        try {
            ...
            ex.printStackTrace(); // Affiche la stacktrace complète dans la console terminal
            UiDialogs.warn("Erreur [" + ex.getClass().getSimpleName() + "] : " + ex.getMessage());
        }
    }
    ...
    private void handleModifier() {
        if (!form.valider())
            return;
        if (!UiDialogs.confirm("Modifier la matière " + form.resolveCode() + " ?"))
            return;
        try {
    ...
    private void handleSupprimer() {
        if (!UiDialogs.confirm("Supprimer " + form.resolveCode() + " ?"))
            return;
        try {
    ...
    private void handleClear() {
        if (!UiDialogs.confirm("Réinitialiser le formulaire ?"))
            return;
        clearForm();
    }
