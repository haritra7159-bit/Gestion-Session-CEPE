...
public class MatiereController implements Initializable {
    ...
    @Override public void initialize(URL location,ResourceBundle resources){
        ...
        tableMatieres.setItems(data);tableMatieres.getSelectionModel().selectedItemProperty().addListener((obs,old,val)->{if(val!=null)fillForm(val);});refresh();
    }
    ...
    @FXML private void handleAjouter(){service.ajouter(new Matiere(txtNumMat.getText(),txtDesignMat.getText(),spCoef.getValue()));refresh();handleClear();}
    @FXML private void handleModifier(){service.modifier(new Matiere(txtNumMat.getText(),txtDesignMat.getText(),spCoef.getValue()));refresh();}
    @FXML private void handleSupprimer(){service.supprimer(txtNumMat.getText());refresh();handleClear();}
    @FXML private void handleClear(){txtNumMat.clear();txtDesignMat.clear();spCoef.getValueFactory().setValue(1);}
