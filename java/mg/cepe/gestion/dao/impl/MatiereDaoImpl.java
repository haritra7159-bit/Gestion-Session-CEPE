...
public class MatiereDaoImpl implements MatiereDao {
    ...
    @Override public List<Matiere> findAll(){String sql="SELECT * FROM matiere ORDER BY designMat";List<Matiere>list=new ArrayList<>();try(Connection c=DatabaseConfig.getDataSource().getConnection();Statement st=c.createStatement();ResultSet rs=st.executeQuery(sql)){while(rs.next())list.add(map(rs));}catch(SQLException e){throw new DatabaseException("Erreur listage matières",e);}return list;}
