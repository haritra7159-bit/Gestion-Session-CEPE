...
public class DeliberationServiceImpl implements DeliberationService {
    ...
    @Override public double calculerMoyenne(String numEleve,String annee){
        ...
        Map<String,Integer> coefs=matiereDao.findAll().stream().collect(Collectors.toMap(Matiere::getNumMat,Matiere::getCoef,(a,b)->a));
        double tp=0;int tc=0;
        for(Note n:notes){int c=coefs.getOrDefault(n.getNumMat(),1);tp+=n.getNote()*c;tc+=c;}
    ...
    @Override public List<ResultatEleve> listerReussis(String annee){
        return deliberer(annee).stream().filter(r->r.getMoyenne()>=9.75).sorted(Comparator.comparingDouble(ResultatEleve::getMoyenne).reversed()).collect(Collectors.toList());
    }
    @Override public List<ResultatEleve> listerEchoues(String annee){
        return deliberer(annee).stream().filter(r->r.getMoyenne()<9.75).sorted(Comparator.comparingDouble(ResultatEleve::getMoyenne).reversed()).collect(Collectors.toList());
    }
    @Override public List<ResultatEleve> listerAdmisSixieme(String annee){
        return deliberer(annee).stream().filter(r->r.getMoyenne()>12.0).sorted(Comparator.comparingDouble(ResultatEleve::getMoyenne).reversed()).collect(Collectors.toList());
    }
    ...
    private ResultatEleve build(Eleve e,String annee){
        ...
        r.setNumEleve(e.getNumEleve());r.setNom(e.getNom());r.setPrenom(e.getPrenom());
        r.setNomEcole(nomEcole);r.setMoyenne(moy);r.setDecision(decision);r.setAdmisSixieme(moy>12.0);
        return r;
    }
}
